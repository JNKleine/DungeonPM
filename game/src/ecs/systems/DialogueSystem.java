...
public class DialogueSystem extends ECS_System {
    ...
    public void update() {
        if (Game.dialogueIsOn) {
            if (Game.dialogueMenu.getUserText() != null && DialogueMenu.sayIsClicked) {
                ...
                Game.callDialogue(
                        callAnswerFromEntity(currentTextFromPlayer), textInputAfterFirstShownIsOn);
     ...
     * Call the DialogHUD
     *
     * @param answerFromEntity The text, the entity to be interacted with. This text is displayed in
     *     the dialog window
     * @param textInputAfterFirstIsOn Determines whether the dialog window redisplays the input text
     *     and the associated button after the player has already used the input text. *
     */
