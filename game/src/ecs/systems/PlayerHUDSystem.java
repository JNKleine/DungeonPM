...
public class PlayerHUDSystem extends ECS_System{
    ...
    public PlayerHUDSystem() {
        ...
        heroQLC = (QuestLogComponent) Game.getHero().get().getComponent(QuestLogComponent.class).get();
    ...
    public void update() {
        ...
        ||(currentMainQuest != heroQLC.getMainQuest() ||
            (heroQLC.getMainQuest() != null && (!heroQLC.getMainQuest().getShortQuestDescriptionWithProgress().equals(mainQuestDescription) ||
            heroQLC.getMainQuest().isQuestIsFinished())))) {
            currentLiveFromHero = heroHC.getCurrentHealthpoints();
            curItem = heroIC.getCurMainItem();
            currentMainQuest = heroQLC.getMainQuest();
            if(currentMainQuest != null) mainQuestDescription = heroQLC.getMainQuest().getShortQuestDescriptionWithProgress();
            moneyOfHero = curHero.getMoney();
