...
/**
 * Shows graphically the current health points of the associated Entity (player) in the bottom left
 * corner
 */
public class PlayerHUD<T extends Actor> extends ScreenController<T> {
    ...
    public void createPlayerHUD(HealthComponent hc, InventoryComponent ic, QuestLogComponent qLC) {
        ...
        ScreenText currentQuest =
                new ScreenText(
                        qLC.getMainQuest() != null
                                ? qLC.getMainQuest().getShortQuestDescriptionWithProgress()
                                : "",
                        new Point(20, 450),
                        1,
                        new LabelStyleBuilder(FontBuilder.DEFAULT_FONT)
        ...
        if (ic.getCurMainItem() != null) {
            ScreenImage curItemImg =
                    new ScreenImage(
                            ic.getCurMainItem().getInventoryTexture().getNextAnimationTexturePath(),
                            new Point(0, 0));
